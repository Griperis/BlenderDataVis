...
class DV_Preferences(bpy.types.AddonPreferences):
    ...
    ui_space_type: bpy.props.StringProperty(
        name="Space Type", default="VIEW_3D", update=update_space_type
    )

    ui_category: bpy.props.StringProperty(
        name="Panel Category", default="DataVis", update=update_category
    )

    debug: bpy.props.BoolProperty(name="Toggle Debug Options", default=False)
    ...
    def get_example_data(self, context):
        ...
        for file in os.listdir(
            os.path.join(get_example_data_path(), self.example_category)
        ):
