...
class OBJECT_OT_BarChart(OBJECT_OT_GenericChart):
    ...
    bar_size: bpy.props.FloatVectorProperty(
        name="Bar size", size=2, default=(0.05, 0.05)
    )
    ...
    def poll(cls, context):
        dm = DataManager()
        return dm.is_type(DataType.Numerical, [2, 3]) or dm.is_type(
    ...
    def draw(self, context):
        ...
        box.label(
            text="Bar Chart Settings:", icon_value=icon_manager.get_icon_id("bar_chart")
        )
        col = box.column(align=True)
        col.prop(self, "use_obj")
        if self.use_obj == "Custom":
            col.prop_search(self, "custom_obj_name", context.scene, "objects")
    ...
    def execute(self, context):
        ...
        for i, entry in enumerate(self.data):
            ...
            if self.use_obj == "Bar" or (
                self.use_obj == "Custom" and self.custom_obj_name == ""
            ):
