    ...
    def create(
        ...
        if dim not in [2, 3]:
            raise AttributeError(
                "Only 2 or 3 dim axis supported. {} is invalid number".format(dim)
            )
        ...
        for i in range(dim):
            ...
            axis.create(
                axis_settings.padding,
                offset,
                labels[dir_idx],
                axis_settings.z_position,
                dim == 2,
            )
...
class Axis:
    ...
    def create_ticks(self, start_pos):
        ...
        for value in float_range(self.range[0], self.range[1], self.step):
            tick_location = self.size * (
                start_pos + (value - self.range[0]) / (self.range[1] - self.range[0])
            )
            self.create_tick_mark(tick_location)
            if len(self.tick_labels) == 0:
                self.create_tick_label(value, tick_location)
            else:
                self.create_tick_label(
                    self.tick_labels[int(value)], tick_location, rotate=True
                )
    ...
    def create(self, padding, offset, label, position="FRONT", only_2d=False):
        ...
        if (self.dir == AxisDir.X and position == "RIGHT") or (
            self.dir == AxisDir.Y and position == "BACK"
        ):
