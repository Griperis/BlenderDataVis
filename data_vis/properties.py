...
class DV_AxisPropertyGroup(bpy.types.PropertyGroup):
    ...
    x_step: bpy.props.FloatProperty(
        name="Step of x axis", default=DataManager().get_step_size("x")
    )
    ...
    y_step: bpy.props.FloatProperty(
        name="Step of y axis", default=DataManager().get_step_size("y")
    )
    ...
    z_step: bpy.props.FloatProperty(
        name="Step of z axis", default=DataManager().get_step_size("z")
    )
    ...
    number_format: bpy.props.EnumProperty(
        name="Format",
        items=(("0", "Decimal", "123.456"), ("1", "Scientific", "1.23e+05")),
    )

    decimal_places: bpy.props.IntProperty(name="Decimal places", default=2, min=0)
...
class DV_HeaderPropertyGroup(bpy.types.PropertyGroup):
    ...
    text: bpy.props.StringProperty(name="Text", default="None")
...
class DV_LabelPropertyGroup(bpy.types.PropertyGroup):
    ...
    create: bpy.props.BoolProperty(name="Create labels", default=True)

    from_data: bpy.props.BoolProperty(name="From data", default=True)

    x_label: bpy.props.StringProperty(name="X Label", default="X")

    y_label: bpy.props.StringProperty(name="Y Label", default="Y")

    z_label: bpy.props.StringProperty(name="Z Label", default="Z")
...
class DV_AnimationPropertyGroup(bpy.types.PropertyGroup):
    ...
    key_spacing: bpy.props.IntProperty(name="Keyframe spacing", default=20, min=1)


class DV_LegendPropertyGroup(bpy.types.PropertyGroup):
    create: bpy.props.BoolProperty(name="Create Legend")
